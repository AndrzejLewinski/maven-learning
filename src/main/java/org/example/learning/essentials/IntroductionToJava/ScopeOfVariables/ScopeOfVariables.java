...
public class ScopeOfVariables {
    ...
    public static void main(String[] args) {
        ...
        scopeOfVariables.digitalWallet();
    ...
    private void digitalWallet() {

        DigitalWallet Jan = new DigitalWallet(1000);
        DigitalWallet Ada = new DigitalWallet(100);

        Jan.addFunds(1000);
        Ada.addFunds(100);

        List<Integer> collect
                = IntStream.range(0, 10).map(i -> ThreadLocalRandom.current()
                .nextInt(20)).boxed()
                .collect(Collectors.toList());

        for(int i =0;i<4;i++){
            try{
                Jan.spendFunds(collect.get(i));
                System.out.println("Jan balance = "+Jan.balance);
                System.out.println("Jan balance = "+Jan.balance+" - "+collect.get(i));
        ...
        for(int i =0;i<4;i++){
            try{
                Jan.transferTo(Ada,collect.get(i));
                System.out.println("Jan balance = "+Jan.balance);
                System.out.println("Jan balance = "+Jan.balance+" - "+collect.get(i));
                System.out.println("Ada balance = "+Ada.balance);
            }catch (IllegalArgumentException e){
                System.out.println("wrong...");
            }
        }
