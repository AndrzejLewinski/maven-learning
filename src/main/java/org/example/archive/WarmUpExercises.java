...
public class WarmUpExercises {
    ...
    private static void justPrinting(double variable){
        System.out.println("Area of the circle is" +" : "+variable);
    }

    private static void listTextStringPrinting(List<String> list){
        list.forEach(System.out::print);
    }

    private static List<Integer> generateTwentyRandomNumbers(){
        return IntStream.range(0,20).map(i -> ThreadLocalRandom.current().nextInt(100)).boxed().collect(Collectors.toList());
    }

    private static List<String> generateStringTenNumbers(){
    ...
    private static void circleArea(){
        ...
        printing("Enter the radius of the circle : ");
        ...
        while (isPositive){
            ...
            try{
                ...
                if(radius>=0){
                    ...
                    printing("Enter valid number: ");
                }
            }catch (InputMismatchException e){
                printing("Invalid input. Please enter a numeric value.");
                scanner.nextLine();
            }
        }

        double circleArea = Math.PI *radius*radius;
        justPrinting(circleArea);
    }
    @SuppressWarnings("unused")
    private void backup(){

        printNumber();
        List<String> archivList = generateStringTenNumbers();
        listTextStringPrinting(archivList);
        List<Integer> collect = generateTwentyRandomNumbers();
